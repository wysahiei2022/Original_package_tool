...
for APEX in $APEXES; do
    ...
    7zz e "$APEXDIR/$APEX" apex_payload.img apex_pubkey -o"$APEXDIR/$APEXNAME" 2>/dev/null >> "$TMPDIR"/zip.log
    7zz x "$APEXDIR/$APEXNAME/apex_payload.img" -o"$APEXDIR/$APEXNAME" 2>/dev/null >> "$TMPDIR"/zip.log
